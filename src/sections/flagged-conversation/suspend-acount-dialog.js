...
export default function SuspendAccountDialog({ suspendAccountDialog }) {
  ...
  const onSubmit = async (data) => {
    ...
    suspendAccountDialog.onFalse();
  };
  return (
    <FormProvider {...methods}>
      <Dialog
        open={suspendAccountDialog.value}
        onClose={suspendAccountDialog.onFalse}
        PaperProps={{
          sx: {
            width: '60%', // or 500
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            <Button
              variant="outlined"
              onClick={suspendAccountDialog.onFalse}
              // disabled={loading}
...
SuspendAccountDialog.propTypes = {
  suspendAccountDialog: PropTypes.object,
};
